...
from src.evaluate.cls_eval import paired_t_test
...
from src.model.WDER import WDER
from src.model.DEKD_1 import DEKD_1
from src.model.DEKD import DEKD


def train_model(model_name, X_train, y_train, X_val=None, y_val=None, cols=None, args=None):
    if model_name == 'LR':
        ...
        pool_classifiers = RandomForestClassifier().fit(X_train, y_train)
    ...
    # SOTA
        ...
        model = DELAK(n_clusters=3, sample_ratio=0.99)

    elif model_name == 'WDER':
        model = WDER(n_clusters=3, sample_ratio=0.99)

    # Our models
    elif model_name == 'DEKD (Stage 1)':
        model = DEKD_1(cols=cols, args=args)

    elif model_name == 'DEKD':
        model = DEKD(cols=cols, args=args)
