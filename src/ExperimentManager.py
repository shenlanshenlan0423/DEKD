...
from src.evaluate.cls_eval import model_evaluate
...
class ExperimentManager:
    ...
    def print_results(self, evaluation_results):
        ...
        metrics = ['Accuracy', 'Precision', 'Recall', 'Sensitivity', 'Specificity', 'F1', 'Balanced Accuracy', 'AUROC', 'AUPRC']
        # metrics = ['AUROC', 'AUPRC']
    ...
    def run(self, load_model):
        ...
        for idx, fold_idx in enumerate(tqdm(list(self.dataset.keys()))):
            ...
            train_set, val_set, test_set = fold_data['train'], fold_data['val'], fold_data['test']
            cols = fold_data['cols']
            ...
            if not load_model:
                ...
                dekd = train_model(model_name='DEKD', X_train=X_train, y_train=y_train, X_val=X_val,
                                   y_val=y_val, cols=cols, args=self.args)
                my_models[fold_idx] = {
                    'DEKD (Stage 1)': dekd_1,
                    'DEKD': dekd,
                }
...
class Experiment_part3(ExperimentManager):
    ...
    def run(self, load_model):
        ...
        for idx, fold_idx in enumerate(tqdm(list(self.dataset.keys()))):
            ...
            y_train, y_val, y_test = train_set[:, -1], val_set[:, -1], test_set[:, -1]
            if not load_model:
                # The dynamic ensemble selection methods
...
class Experiment_part4(ExperimentManager):
    def __init__(self, dataset_path, model_path, results_path, args):
        super().__init__(dataset_path, model_path, results_path, args)
    ...
    def run(self, load_model):
        ...
        for idx, fold_idx in enumerate(tqdm(list(self.dataset.keys()))):
            ...
            if not load_model:
                ...
                wder = train_model(model_name='WDER', X_train=X_train, y_train=y_train, args=self.args)
                trained_models[fold_idx] = {
                    'DELAK': delak,
                    'WDER': wder,
                }
            else:
                ...
                wder = trained_model_dict['WDER']
            evaluation_result = {
                'DELAK': model_evaluate(model=delak, X_test=X_test, y_test=y_test),
                'WDER': model_evaluate(model=wder, X_test=X_test, y_test=y_test),
            }
            evaluation_results.append(evaluation_result)
        self.print_results(evaluation_results)
        if not load_model:
...
def parse_args(outcome_label, balance_ratio, n_clusters, Eta, Alpha, Beta, Lambda):
